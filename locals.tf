...
locals {
  ...
  # Validation: Check that exactly one destination is specified
  ...
  # Determine effective destination for output
  effective_source = (
  ...
  tags = merge(
    {
      "ManagedBy" = "tf-aws-module_primitive-vpc_security_group_egress_rule"
    },
    var.tags,
  )
}
...
check "source_validation" {
  assert {
    ...
    error_message = "Exactly one destination must be specified: cidr_ipv4, cidr_ipv6, prefix_list_id, or referenced_security_group_id."
  }
}
