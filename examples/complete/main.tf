...
resource "aws_security_group" "test" {
  ...
  description = "Test security group for egress rule module"
  vpc_id      = aws_vpc.test.id
...
module "egress_ssh_ipv4" {
  ...
  description       = "Allow to specific IPv4 CIDR"
}

# Example 2: HTTPS access from specific IPv4 CIDR
module "egress_https_ipv4" {
  source = "../../"
