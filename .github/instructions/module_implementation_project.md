# Requirements: Terraform Primitive Module — `aws_vpc_security_group_egress_rule`

> Initial draft for an AI coding agent. Updated to reflect framework conventions and resource capabilities.

## 1. Purpose & Scope
Create a **Terraform primitive module** that manages a single AWS VPC **security group egress rule**, modeled on the reference primitive module style/framework:
- Reference primitive: [`tf-aws-module_primitive-iam_role`](https://github.com/launchbynttdata/tf-aws-module_primitive-iam_role)
- Target resource: [`aws_vpc_security_group_egress_rule` (AWS provider v5.100.0)](https://registry.terraform.io/providers/hashicorp/aws/5.100.0/docs/resources/vpc_security_group_egress_rule)
...
- **security_group_id** *(string, required)* — Target SG for the egress rule.
...
- `egress_rule_effective_source` — Canonicalized string describing the active source (CIDR/prefix list/SG).
...
1. **minimal_cidr_ipv4** — Create an SG and a single IPv4 egress rule for SSH.
2. **ipv6_http** — Similar, but for IPv6 and port 80.
3. **prefix_list** — Use prefix list for HTTPS.
4. **sg_to_sg** — Peer SG egress for Postgres.
...
- Update the test implementation in `tests/testimpl/test_impl.go` to validate creation of the expected **Security Group egress Rule** resource.
...
|------|------|----------|-------------|
| security_group_id | string | yes | ID of the SG to attach this egress rule to |
...
| egress_rule_effective_source | Canonical source descriptor (CIDR/PL/SG) |
...
- Rule exists and is type "egress"
...
**Response:** Add `aws_default_security_group` resource to all examples that create VPCs. Configure it with no egress or egress rules to explicitly deny all traffic on the default security group, following AWS security best practices (FG_R00089 compliance).
...
1. **complete** - Multiple egress rules with different sources and ports
...
- ✅ `TestSecurityGroupegressRuleExists` - Validates rule presence
- ✅ `TestSecurityGroupegressRuleProperties` - Validates rule configuration
